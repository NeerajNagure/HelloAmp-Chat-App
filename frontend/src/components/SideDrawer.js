...
const SideDrawer = () => {
  ...
  const logoutHandler = () => {
    localStorage.removeItem("HelloAmp");
    navigate("/");
  };
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${process.env.REACT_APP_SERVER}/api/users?search=${search}`, config);
  ...
  const accessChat = async (userId) => {
    ...
    try {
      ...
      const { data } = await axios.post(`${process.env.REACT_APP_SERVER}/api/chats`, { userId }, config);
  ...
  const removeNotification = async (id) => {
    try {
      ...
      await axios.delete(`${process.env.REACT_APP_SERVER}/api/notification/${id}`, config);
    } catch (err) {
      toast.error(err);
    }
  };
  ...
  const getPrevNotification = async () => {
    try {
      ...
      const { data } = await axios.get(`${process.env.REACT_APP_SERVER}/api/notification`, config);
